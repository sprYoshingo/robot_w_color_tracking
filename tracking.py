...
def main():
    ...
    while True: #read from the cam until it fails / ends
        ...
        if contours:
            ...
            if cv2.contourArea(largest) > 100: #can be used to hide tiny spaces but current doesnt do that
...
def get_direction(dx,dy):
    ...
    #forward if dy is pos and bacwards is dy is negative
    if dy >0:
        direction.append("S")
    elif dy<0:
        direction.append("N")

    #right if dx is pos and left if dx is negative
    if dx>0:
        direction.append("E")
    elif dx<0:
        direction.append("W")
